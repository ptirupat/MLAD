# Description : Baseline model without class-specific features. 
...
class VideoTransformer(nn.Module):
    def __init__(self, num_clips, num_classes, feature_dim, hidden_dim, num_layers):
        ...
        self.classifier = nn.Sequential(nn.Linear(feature_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, num_classes))

    def forward(self, features):
        activations = {}
        features = self.positional_encoding(features)
        initial_outputs = self.classifier(features)
